...
#define SET_TRI_DATA_RDY(tri) ATOMIC_BLOCK { tri.dataTimestamp = esp_timer_get_time(); tri.dataReady = true; }
...
void loop() {
  ...
  if(GET_TRI_DATA_RDY(tri_a)) {
    ...
    ATOMIC_BLOCK {
      capture_time = tri_a.dataTimestamp;
  ...
  if(GET_TRI_DATA_RDY(tri_b)) {
    int64_t capture_time;
    ATOMIC_BLOCK {
      capture_time = tri_b.dataTimestamp;
